...
- **新增**: 20 单元测试 (1667 → 1687), 1 新服务 + 2 定时任务 + 2 API 端点
...
- **新增**: 19 单元测试 (1687 → 1706), 2 新组件 + 1 新页面 + 2 API 端点
...
- ✅ **后端 CRUD** — worldbook + entry 完整 CRUD API (9 端点 + 导入/导出)
...
- ✅ **Pinia Store** — useWorldBookStore 完整状态管理
- ✅ **i18n** — worldBooks.* 新增 15 键 (en-US + zh-CN)
- **新增**: 7 单元测试 (1720 → 1727), 1 新页面 + 1 新 store + 11 API 端点

### 迭代 23: 聊天体验优化 ✅ (2026-02-22)
- ✅ **消息搜索** — ILIKE 全文搜索 + 聊天窗口搜索栏 (防抖 300ms)
- ✅ **聊天导出** — JSON/Markdown/TXT 三种格式导出 (Blob 下载)
- ✅ **对话回溯** — 回溯到指定消息 (删除后续消息 + 确认对话框)
- ✅ **消息收藏** — message_bookmarks 表 + 收藏切换 + 侧边栏收藏面板
- ✅ **聊天模板** — chat_templates 表 + CRUD API + 欢迎页模板选择器
- ✅ **i18n** — chat.* 新增 30+ 键 (en-US + zh-CN)
- **新增**: 2 新表 + 2 新 store + 6 新 API 端点 + 1727 测试通过

### 迭代 24: 角色个性化增强 ✅ (2026-02-23)
- ✅ **语音定制** — VoiceConfig 接口 (语言/语速/音调/声音) + VoiceSettings 组件 + CharacterEditor 集成
- ✅ **情感散点图** — ECharts 2D Valence-Arousal 散点图 + 象限标签 + 时间线/图谱切换
- ✅ **角色关系** — character_relationships 表 + CRUD API + ECharts 力导向关系图 + 添加/编辑对话框
- ✅ **角色成长** — character_growth 表 + XP/等级自动计算 + GrowthPanel 组件 + 里程碑系统
- ✅ **成长联动** — 发送消息 +10 XP, 创建聊天 +25 XP (fire-and-forget)
- ✅ **i18n** — character.* 新增 45+ 键 (关系/成长/语音/情感, en-US + zh-CN)
- **新增**: 2 新表 + 4 新 API 路由 + 3 新组件 + 1733 测试通过

### 迭代 25: AI 对话增强 ✅ (2026-02-23)
- ✅ **模型注册表** — ModelMeta (contextWindow, maxOutputTokens, defaultTemperature) + 10 内置模型元数据
- ✅ **Token 计数升级** — CJK 感知分词估算 (替代 text.length/4 粗略估算)
- ✅ **上下文窗口管理** — ContextManager 滑动窗口策略 (系统提示 + 最近消息 + token 预算裁剪)
- ✅ **多模型切换** — PATCH /chats/:id/model + ChatWindow 模型选择下拉框 + 上下文窗口大小显示
- ✅ **流式中止** — ABORT_GENERATION WS 消息 + AbortController + 保存部分内容 + Stop 按钮
- ✅ **对话分支** — parentMessageId 列 + 递归 CTE 分支查询 + 兄弟消息导航 (◀ 1/3 ▶)
...
| API 端点 | 120+ |
| 数据库表 | 26 张 (PostgreSQL) + 8 张 (ClickHouse) |
